      ...
      <DialogContent className="sm:max-w-[400px] border border-white/10 bg-[#0B0E14] p-0 shadow-2xl gap-0">
        ...
        <div className="p-6 pb-2 flex flex-col items-center text-center">
          ...
          <div className="mt-4 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-3 py-1.5">
            <p className="text-[11px] font-medium text-emerald-400 flex items-center gap-1.5">
              ...
              70% of users found this helpful
            </p>
          </div>
        </div>
        ...
        <DialogFooter className="p-6 pt-2 flex flex-col gap-2 sm:flex-col sm:space-x-0">
          ...
          <Button
            ...
            onClick={() => {
              // Navigate to results with the base suggested roles
              const suggestedRolesStr = sessionStorage.getItem("suggestedJobRoles");
              if (suggestedRolesStr) {
                const suggestedRoles = JSON.parse(suggestedRolesStr);
                // Format the data to match the expected structure for results page
                const baseRecommendations = {
                  recommendations: suggestedRoles.map((role: any, index: number) => ({
                    career: role,
                    matchScore: role.similarity ? (role.similarity * 100) : (role.match_percentage || 85),
                    similarity: role.similarity,
                    reasoning: `Based on your skills and background. ${role.recognized_skills?.length || 0} skill${role.recognized_skills?.length === 1 ? '' : 's'} matched.`
                  })),
                  analysisTime: 0,
                  isBaseRecommendation: true // Flag to indicate this is from onboarding, not test
                };
                sessionStorage.setItem("careerRecommendations", JSON.stringify(baseRecommendations));
                router.push('/results');
              } else {
                // If no suggested roles, just close the dialog
                setOpen(false);
              }
            }}
          >
